...
// ── Session token (single slot, RAM only) ─────────────────────────────────
// 32 hex chars + null terminator. Cleared on reboot.
#define SESSION_TOKEN_LEN 32
static char sessionToken[SESSION_TOKEN_LEN + 1] = {0};

// Generate a new random token using esp_random()
...
static bool isAuthorised(httpd_req_t *req) {
    if (sessionToken[0] == '\0') return false;   // no active session

    char tokenBuf[SESSION_TOKEN_LEN + 1] = {0};
    if (httpd_req_get_hdr_value_str(req, "X-Auth-Token",
                                    tokenBuf, sizeof(tokenBuf)) != ESP_OK) {
        return false;
    }
    // Constant-time compare
    uint8_t diff = 0;
    for (int i = 0; i < SESSION_TOKEN_LEN; i++) {
        diff |= (uint8_t)sessionToken[i] ^ (uint8_t)tokenBuf[i];
    }
    return diff == 0;
}

// Convenience: send a 401 JSON error
...
void webHandlerHook(webServerMacro hook)
{
    ...
    {
    case BASE_HTML:
        Serial.println("Shell requested");
        break;
    case LOGIN_HTML:
        Serial.println("Login page requested");
        break;
    case DASHBOARD_HTML:
        Serial.println("Dashboard fragment requested");
        break;
    case WIFI_CONFIG_HTML:
        Serial.println("WiFi config fragment requested");
        break;
    default:
        break;
    }
...
char* apiLoginHandlerHook(httpd_req_t *req) {
    ...
    if (json == NULL) {
        ...
        if (!cJSON_IsString(pass_j)) {
            ...
            // Pad attempt to ADMIN_PASS_LEN for constant-time compare
...
// POST /api/logout  (no body needed; requires valid token)
char* apiLogoutHandlerHook(httpd_req_t *req) {
    if (!isAuthorised(req)) {
        sendUnauthorised(req);
        return nullptr;
    }
...
char* apiAuthChangePasswordHandlerHook(httpd_req_t *req) {
    ...
    if (json == NULL) {
        ...
        if (!cJSON_IsString(cur_j) || !cJSON_IsString(new_j)) {
            cJSON_AddBoolToObject(response, "success", false);
            cJSON_AddStringToObject(response, "message", "current and new password fields required");
        } else {
            char attempt[ADMIN_PASS_LEN + 1] = {0};
            ...
            if (!checkAdminPassword(attempt)) {
                ...
                // Invalidate active session — user must log in again
                memset(sessionToken, 0, sizeof(sessionToken));
                cJSON_AddBoolToObject(response, "success", true);
...
char* apiWifiConnectHandlerHook(httpd_req_t *req) {
    ...
    if (json == NULL) {
        ...
        if (cJSON_IsString(ssid_j)) {
            ...
            if (WiFi.status() == WL_CONNECTED) {
                // Save to EEPROM (re-use existing SSID/PASS slots)
                for (int i = 0; i < 32; i++)
...
// GET /api/devices — return all devices and their current state
...
char* apiDevicesAddHandlerHook(httpd_req_t *req) {
    ...
    if (json == NULL) {
        ...
        if (cJSON_IsString(name_j) && cJSON_IsNumber(pin_j)) {
            ...
            devices[idx].state = 0; // default off
            deviceCount++;
    ...
    if (json == NULL) {
        ...
        if (cJSON_IsNumber(idx_j)) {
            ...
            if (idx < 0 || idx >= (int)deviceCount) {
                ...
                // Turn off the pin before removing
                digitalWrite(devices[idx].pin, LOW);
