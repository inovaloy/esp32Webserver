...
#define ADMIN_PASS_LEN    32    // max admin password length (bytes stored)

// Maximum number of controllable GPIO devices
#define MAX_DEVICES       16

// EEPROM layout for device storage
#define DEVICE_COUNT_ADDR 210   // 1 byte: number of saved devices
#define DEVICE_BASE_ADDR  220   // start of device slot array
#define DEVICE_SLOT_SIZE  18    // 16 bytes name + 1 byte pin + 1 byte state
#define DEVICE_NAME_LEN   16

// Single controllable GPIO device
struct Device {
    char    name[DEVICE_NAME_LEN];  // human-readable label (null-terminated)
